...
def gh(args, check=False):
    r = subprocess.run(['gh', *args], cwd=CWD, env=env, capture_output=True, text=True)
    if check and r.returncode != 0:
        print('gh error:', ' '.join(args), file=sys.stderr)
        print(r.stdout, r.stderr, file=sys.stderr)
    return r


# 1) Determine base from PR #8 state.
base = 'feat/receipt-capture'
r = gh(['pr', 'view', '8', '--repo', REPO, '--json', 'number,state,baseRefName,headRefName'])
if r.returncode == 0:
    ...
    print('could not read PR #8:', r.stderr.strip())
...
**Stacks on #8** (`feat/receipt-capture`) — base is that branch, so the diff here is only the hardening. Retarget to `main` after #8 merges.
...
- **Local vision model** (`specs/local-vision-model.md`): go/no-go on a ~half-day spike; dev-only vs prod ambition. Recommended default: Ollama + Qwen2.5-VL-7B, adopt for dev immediately if recall is moderate, prod is eval-gated.
...
create = gh(['pr', 'create', '--repo', REPO, '--base', base, '--head', 'feat/orchestration-hardening',
