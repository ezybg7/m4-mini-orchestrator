...
**Dispatch (no reviewer has reported since your last dispatch, or you have not dispatched this round).** First pin the target: `gh pr view <n> --json headRefOid --jq .headRefOid` — the dispatch names that SHA and the reviewers review *that*, not whatever the branch is by the time they start. Post ONE comment @mentioning both, using the exact mention markdown from your roster, so they review in parallel. Give each only what the issue does not already say — the PR number, and any sequencing or focus this round needs; after a fix round, say which findings were supposedly addressed so they check those first. **After a fix round the dispatch is `targeted` by default (2026-09-14):** name the previous round's pinned SHA and the new head, list the findings the implementer reports fixed, and write the word `targeted` — the reviewers then review the diff between the two SHAs plus that list and re-run no gate that `gh pr checks <n>` shows green for the head. Write `full pass` instead when the implementer's HANDOFF `did:` or `stale-risk:` names shared code (a component many screens use, a data-layer seam, a migration renumber, a resolve-with-main) or when the previous round surfaced a High. Round 1 is always a full pass. Add `mode <full|targeted>` to the instrumentation line so the change can be judged on evidence. Then `multica squad activity <issue> action --reason "dispatched round N"` and end your turn. Leave the status at `in_review` throughout — that column is yours, and moving it would hand the issue to someone else mid-review.
...
**Post a PACK after every consolidation (2026-09-15, spec 61 §Context packs — invoke `pantry-context-pack`).** A new root comment `PACK · <head>` in the skill's shape: keep the implementer's `spec:`, `heads:`, `blast:` and `ci:` lines, set `findings:` to the survivors by id and state, add the round's thread to `skip:` once it is resolved, and write `read:` for the next role (the implementer's fix list, or the reviewers' targeted diff). Your dispatch comment names the pack ("read the PACK · <head> first").
